...
// import { calcOfferInfoSize, calcOfferLogoSize, calcOfferTitleSize, calcOfferMainImageSize, calcOfferSkuSize, calcOfferAttributesSize } from "@/components/LayerOfferElement/calcOffer";
...
export async function queryOfferBy(offerLinkList: Array<string | number>): Promise<TypeOfferMaterialResult[]> {
  const endpoint = `/offerCollection/link/parse`;
  ...
  try {
    const response = await request(endpoint, {
      ...
      body: JSON.stringify({ offerLinkList }),
      credentials: 'include',
    });
    ...
    if (!data || data.success !== true || !data.result) {
      throw new Error(data?.retMsg || 'queryOfferBy failed');
    }
    ...
    throw new Error(e?.message || 'queryOfferBy failed: invalid json response');
  }
}
