...
export default function RiderManage() {
  ...
  const handleSubmit = async () => {
    ...
    try {
      editing ? await put(`/riders/${editing.id}`, v) : await post('/riders', v);
  ...
  return (
    <>
      <DynamicTable
        ...
        children={<Button type="primary" onClick={() => { setEditing(null); form.resetFields(); setModalOpen(true); }}>新增骑手</Button>}
        renderActions={(_, r) => (
          <>
            <Button type="link" size="small" onClick={() => { setEditing(r); form.setFieldsValue(r); setModalOpen(true); }}>编辑</Button>
            <Button type="link" size="small" danger onClick={async () => { await del(`/riders/${r.id}`); message.success('删除成功'); setRefreshKey(k => k + 1); }}>删除</Button>
          </>
        )}
      />
