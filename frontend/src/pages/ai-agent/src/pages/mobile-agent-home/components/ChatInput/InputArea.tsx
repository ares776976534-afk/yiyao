...
export default ({
  value,
  onChange,
  placeholder = $t("global-1688-ai-app.mobile-agent-home.ChatInput.InputArea.ncn", "你可以向我继续提问..."),
  handleEnter = () => { },
  ...
  const handleChange = () => {
    if (contentEditableRef.current) {
      ...
      // 检查是否包含HTML元素，如果有，则清除格式
      // 但不要直接修改DOM，避免影响输入法上下文
      const htmlContent = contentEditableRef.current.innerHTML;
      if (htmlContent.includes('<') && htmlContent.includes('>') && htmlContent !== text) {
        // 使用execCommand来清除格式，而不是直接设置innerText
  ...
  //     console.log('handleInputContentClick', e);
