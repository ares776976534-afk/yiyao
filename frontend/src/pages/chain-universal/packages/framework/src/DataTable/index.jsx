...
function DataTable({
  ...
  const componentInfo = useRef({
    componentType: "default",
    componentListQueryFn: listQueryFn,
  });
  const allSelectedData = useRef({});
  const column = schema["colSchema"] ? schema["colSchema"](currentStatus) : []; // 根据status去columns里选择表头组合
  const batchActions = schema["colSchema"]
    ? schema["batchActionSchema"](currentStatus)
    : {}; // 根据status去选择批量操作btns
  const {
    hasRowSelection,
    leftAction,
  ...
  const emptySlot = schema["emptySchema"]
    ? schema["emptySchema"](currentStatus, query, dataSource)
    : null; // 根据status去选择批量操作btns
      ...
      componentInfo.current
        ...
        .then(({ model, total }) => {
          if (model) {
            ...
            expandedRowRender &&
              isExpandedDefault &&
              setOpenRowKeys([model?.[0]?.[primaryKey]]);
          } else {
            setDataSource([]);
          }
  ...
  useEffect(() => {
    ...
    if (
      ...
      componentInfo.current = {
        componentType,
        componentListQueryFn,
        render: crt.render,
        wrapStyle: crt.wrapStyle,
      };
    }
    ...
    reloadTable((reloadQuery = {}, clearChecked) =>
      run({ ...query, pageNo, pageSize, ...reloadQuery }, clearChecked)
    );
  ...
  const emptyRender = (type) => {
    return (
      <div
        className="empty-state"
        style={{
          height: type === "table" ? "auto" : "560px",
        }}
      >
        {emptySlot || (
          <>
            <img
              src="https://img.alicdn.com/imgextra/i1/O1CN0121Us1J1dhnOOq8XVS_!!6000000003768-2-tps-297-287.png"
              alt="img"
              style={{ width: "75px", height: "72px" }}
            />
            <div className="text-[14px] mt-[16px] text-[#999]">
              暂无相关数据展示
            </div>
          </>
        )}
  ...
  const renderColumns = (columns) => {
    return columns.map((col) => {
      ...
      // 如果是普通列，直接渲染 Table.Column
      return colType === "group" ? (
        <Table.GroupHeader key={__col.title} {...__col} />
      ) : (
        <Table.Column key={__col.title} {...__col} />
      );
    });
  };
  ...
  return (
    <Loading tip="加载中..." visible={tableLoading} className="dataTable">
        ...
        <div className="dataTable-header">
          <div className="dataTable-header-items">
            {hasRowSelection && showSelectTip ? (
              <div className="dataTable-header-rowSelect">
                已选
                <span className="dataTable-header-rowSelect-num">
          ...
          {rightAction ? (
            <div key={currentStatus} className="dataTable-header-items">
              <div className="dataTable-header-right-action">
                {rightAction({
      ...
      <div className="dataTable-content">
        {componentInfo.current.render &&
        componentInfo.current.componentType === "slot" ? (
          isLoading(
            <div
              style={componentInfo.current.wrapStyle || {}}
                ...
                ? dataSource.map((dataItem, index) =>
                    componentInfo.current.render(
                      ...
                      handleActionClick,
                      {
                        ...
                        currentCheckedRef: currentCheckedRef,
                        currentChecked,
                      }
                    )
                  )
          ...
          <Table.StickyLock
            ...
            rowSelection={
              hasRowSelection
                ? {
                    ...
                    selectedRowKeys: currentCheckedRef.current.map(
            ...
            style={tableStyle}
            emptyContent={emptyRender("table")}
            {...otherTableProps}
          >
            {/* {column.map((col) => {
              let _col = col;
              ...
              return colType === 'group' ? <Table.GroupHeader key={__col.title} {...__col} /> : <Table.Column key={__col.title} {...__col} />;
            })} */}
      ...
      {showPagination ? (
        <div
          className={`dataTable-content-page dataTable-content-page-${otherPagination?.shape}`}
        >
          <Pagination
            total={pageTotal}
            current={pageNo}
