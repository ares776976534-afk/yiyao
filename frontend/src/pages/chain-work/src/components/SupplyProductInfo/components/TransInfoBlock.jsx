...
import { queryTransInfo, queryOfficialAvgFgt } from '../api';
import { dealPrice, Logger } from '@/utlis';
  ...
  const dealTransInfo = (infos, data, postageType, _logisticsId) => {
    return new Promise((resolve, reject) => {
      ...
      infos.forEach((item) => {
        ...
        if (item.key === 'sendAddress' && (!data['sendAddress'] || data['sendAddress'].length === 0)) {
          item.value = '未设置';
        }
        // 不透出【查看运费模板】的入口
