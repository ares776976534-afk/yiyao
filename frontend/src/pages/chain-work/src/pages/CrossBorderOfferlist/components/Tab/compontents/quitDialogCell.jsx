...
const QuitDialogComp = (props) => {
  const [value, setValue] = useState(null);
  const [inputValue, setInputValue] = useState('');
  const [buttonLoading, setButtonLoading] = useState(false);
  const [otherChacked, setOtherChacked] = useState(false);
  const [quitDisabled, setQuitDisabled] = useState(true);
  const { quitDialogParams, updateTableData, setQuitDialogVisibleFunc, visible } = props;
  const { text = '退出货通全球', itemId } = quitDialogParams;
  const textParams = {
    退出货通全球: {
      title: '退出货通全球',
      subTitle: (
  ...
  const actionQuitClick = (_visible) => {
    // setDialogVisible(_visible);
    setQuitDialogVisibleFunc(_visible);
    // 关闭弹窗，清空选项
    if (!_visible) {
      setValue(null);
      setInputValue('');
