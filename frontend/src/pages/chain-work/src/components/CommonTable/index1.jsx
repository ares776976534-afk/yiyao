...
function CommonTable({
  ...
  const [mainStatusFilter, setMainStatusFilter] = useState({ type: 'default', render: null });
  ...
  const handleChangeStatus = (value, ifNeedReloadSearch = true) => {
    if (ifNeedReloadSearch) {
      setSearch(Object.assign({}, { currentStatus: value, pageNo: 1, pageSize })); // 这里search写死了查询接口的入参，建议去mtop配置的地方统一修改。
      setStatus(value);
  ...
  useEffect(() => {
    ...
    setMainStatusFilter(getRenderParams('subStatusFilter', SlotOrShowStatusFilter));
  }, []);
  ...
  const getRenderParams = (componentType, SlotCom) => {
    ...
    if (
      ...
      renderParams = Object.assign(renderParams, componentSchema(status)[componentType], {
        type,
        render: renderType,
      });
      ...
      renderParams = Object.assign(renderParams, { type: 'default', render: defaultCom });
    } else {
      // 否则认为是slot
      renderParams = Object.assign(renderParams, { type: 'slot', render: SlotCom });
    }
    return renderParams;
  };
  ...
  return (
    <div className={`commonTable${tabStyle ? '-noBorder' : ''}`}>
      <Block show={SlotOrShowStatusFilter}>
        {mainStatusFilter &&
      ...
      <Block border={blockBorder} show={schema['filterSchema'](status)?.length > 0}>
        <SearchFilter
          status={status}
      ...
      <Block show={!!showType.messageBar.render}>
        ...
        {showType.messageBar.render && showType.messageBar.render({ status, data: showType.messageBar.data })}
      </Block>
      <Block style={showType.subStatusFilter?.wrapStyle || {}} show={!!showType.subStatusFilter?.render}>
        {showType.subStatusFilter.render && showType.subStatusFilter.render({ status, onChange: handleChangeStatus })}
      </Block>
