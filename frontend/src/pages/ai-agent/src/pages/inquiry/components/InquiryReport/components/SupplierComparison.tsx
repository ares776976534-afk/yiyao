...
import { WantWantIcon, ShopOrderIcon, PositiveReviewIcon } from '@/components/Icon';
import type {
...
function SupplierComparison({ data, tableHead, inquiryAnswers }: TypeSupplierComparisonProps) {
  ...
  const generateColumns = (): TypeTableColumn[] => {
    ...
    // 供应商信息列（固定第一列）
    columns.push({
      title: $t("global-1688-ai-app.inquiry.InquiryReport.SupplierComparison.gft", "供应商信息"),
      ...
      render: (sellerInfo: TypeSellerInfo) => (
        <div className={sellerInfo?.isBest ? undefined : styles.supplierInfoTd}>
          {sellerInfo?.isBest && (
            ...
            //   <span className={styles.recommendText}>{$t("global-1688-ai-app.inquiry.InquiryReport.SupplierComparison.zod", "最佳推荐")}</span>
            // </Tag>
            <Tag color="orange" className={styles.bestRecommendationTag}>
              <AiIcon />
              <span className={styles.bestRecommendationText}>{$t("global-1688-ai-app.inquiry.InquiryReport.SupplierComparison.zod", "最佳推荐")}</span>
            </Tag>
          )}
    ...
    columns.push({
      title: $t("global-1688-ai-app.inquiry.InquiryReport.SupplierComparison.xpd", "询盘完成度"),
      dataIndex: 'progress',
      key: 'progress',
      width: 120,
      fixed: 'right',
          ...
          <Tooltip
            ...
            title={
              <div className={styles.completionTooltip}>
                <div className={styles.completionTooltipTitle}>{$t("global-1688-ai-app.inquiry.InquiryReport.SupplierComparison.xpissue", "询盘问题")}</div>
  ...
  return (
    <div className={styles.supplierComparison}>
      <div className={styles.header}>
        ...
        <h2 className={styles.headerTitle}>{$t("global-1688-ai-app.inquiry.InquiryReport.SupplierComparison.gysdb", "供应商对比")}</h2>
      </div>
